...
2. Для production установить через меню **нативный Telemt с SHA-256**, **Telemt в Docker по OCI digest** либо **MTG с SHA-256**. Устаревшие CLI upstream installers не используются простым меню.
...
Проверяемая установка **Telemt** выполняется через Manager: `[3] УСТАНОВИТЬ
ПРОКСИ` → `[1] Меню Telemt`, затем выберите нативный или Docker-вариант.
Нативный архив проверяется по SHA-256; Docker-образ закреплён по OCI digest.
Процесс, порт и соответствующий firewall проверяются автоматически до вывода
итогового сообщения об успешной установке.

Устаревшие CLI-флаги `-telemt` и `-zig` по умолчанию завершаются с ошибкой,
поскольку upstream installers скачивают вложенные артефакты без закреплённых
checksums. Проверяемый Telemt устанавливается через простое меню; порядок
развёртывания приведён в [DEPLOYMENT_VPS.md](DEPLOYMENT_VPS.md).

### Параметры для автоматической установки
| Флаг                          | Значение по умолчанию      | Описание                             |
| ----------------------------- | -------------------------- | ------------------------------------ |
| `-telemt`                     | —                          | Устаревший CLI upstream installer; для проверяемой установки используйте меню Telemt |
